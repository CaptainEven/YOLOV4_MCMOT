...
def detect_image(net, meta, im, thresh=0.5, hier_thresh=.5, nms=.45, debug=False):
    ...
    if nms:
        do_nms_sort(dets, num, meta.cls_names, nms)
    if debug: print("did sort")
    res = []
    if debug: print("about to range")
    for j in range(num):
        if debug: print("Ranging on " + str(j) + " of " + str(num))
        if debug: print("Classes: " + str(meta), meta.cls_names, meta.names)
        for i in range(meta.cls_names):
            if debug: print("Class-ranging on " + str(i) + " of " + str(meta.cls_names) + "= " + str(dets[j].prob[i]))
            if dets[j].prob[i] > 0:
                b = dets[j].bbox
                if altNames is None:
                    nameTag = meta.names[i]
...
def performBatchDetect(thresh=0.25, configPath="./cfg/yolov3.cfg", weightPath="yolov3.weights",
    ...
    for b in range(batch_size):
        ...
        if nms:
            do_nms_obj(dets, num, meta.cls_names, nms)
        boxes = []
        scores = []
        classes = []
        for i in range(num):
